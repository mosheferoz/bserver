...
router.post('/export-group-to-storage', async (req, res) => {
  try {
    const { sessionId, groupId } = req.body;
    
    if (!sessionId || !groupId) {
      return res.status(400).json({ error: 'נדרש sessionId ו-groupId' });
    }

    const whatsapp = require('../services/whatsapp.service');
    const exportData = await whatsapp.exportGroupToStorage(sessionId, groupId);
    ...
    const file = bucket.file(`files/${exportData.name}`);
    await file.save(exportData.content, {
      contentType: 'text/csv',
      metadata: {
        contentType: 'text/csv',
        metadata: {
          originalName: exportData.name,
    ...
    // שמירת המטא-דאטה ב-Firestore
    const firestore = admin.firestore();
    await firestore.collection('files').add({
      name: exportData.name,
      path: `files/${exportData.name}`,
    ...
    console.error('Error exporting group to storage:', error);
