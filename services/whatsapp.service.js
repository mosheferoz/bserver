...
class WhatsAppService {
  ...
  async checkConnection(sessionId) {
    try {
      ...
      const client = this.clients.get(sessionId);
      if (!client) {
        logger.info(`No client found for session ${sessionId}`);
        return false;
      }

      // בדיקה אם הלקוח מחובר באמת
      try {
        const state = await client.getState();
        const isConnected = state === 'CONNECTED';
        
        if (!isConnected) {
          logger.info(`Client state is ${state}, cleaning up...`);
  ...
  async handleDisconnection(sessionId) {
    try {
      logger.info(`Handling disconnection for session ${sessionId}`);
      ...
      // ניקוי תיקיית האימות
      const authFolder = path.join(process.cwd(), 'whatsapp-auth', `session-${sessionId}`);
      if (fs.existsSync(authFolder)) {
        logger.info(`Cleaning auth folder for session ${sessionId}...`);
        try {
          // מחיקה רקורסיבית של התיקייה
          await fs.promises.rm(authFolder, { recursive: true, force: true });
          logger.info(`Auth folder cleaned successfully for session ${sessionId}`);
        } catch (error) {
          logger.error(`Error cleaning auth folder: ${error}`);
          // ממשיכים גם אם יש שגיאה בניקוי
        }
      }
    } catch (error) {
      logger.error(`Error in handleDisconnection: ${error}`);
    }
  }

  async disconnect(sessionId) {
    try {
      logger.info(`Disconnecting WhatsApp for session ${sessionId}`);
